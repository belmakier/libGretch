...
namespace Gretch {
  int GretinaConf::ReadCrystalMap(std::string fn) {
  ...
  std::vector<double> GretinaConf::GetPosition(int hole, int xtl, double x, double y, double z) const {
  ...
  int GretinaConf::ReadCalibration(std::string fn) {
    std::ifstream file(fn.c_str());
  ...
  int GretinaConf::ReadEfficiencyCal(std::string fn) {
  ...
  float GretinaConf::Efficiency(const double &e) const {
    if ( EfficiencyType == EffType::kLogLogPoly ) {
  ...
  int GretinaConf::ReadCrystalSwap(std::string fn) {
    std::ifstream file(fn.c_str());
    int crst_i, crst_j;
  ...
  int GretinaConf::ReadNoisyCrystals(std::string fn) {
    std::ifstream file(fn.c_str());
    int i, noisy;
    while (file >> i >> noisy) {
  ...
  int GretinaConf::ReadAddbackDets(std::string fn) {
    std::ifstream file(fn.c_str());
    int i, addback;
